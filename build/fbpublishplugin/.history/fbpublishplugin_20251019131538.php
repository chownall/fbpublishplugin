...
class FB_Publish_Plugin {
    ...
    public static function get_options() {
        $defaults = [
            'access_token' => '', // global fallback (deprecated)
            'page_access_token' => '',
            'group_access_token' => '',
            'page_id' => '',
            'group_id' => '',
    ...
    public function register_settings() {
        register_setting(self::OPTION_KEY, self::OPTION_KEY, [
            ...
            'sanitize_callback' => function ($input) {
                ...
                $sanitized['group_access_token'] = isset($input['group_access_token']) ? sanitize_text_field($input['group_access_token']) : '';
        ...
        add_settings_field('group_access_token', __('Jeton d’accès Groupe', 'fbpublishplugin'), function () {
            $options = self::get_options();
            echo '<input type="text" class="regular-text" name="' . esc_attr(self::OPTION_KEY) . '[group_access_token]" value="' . esc_attr($options['group_access_token']) . '" placeholder="EA..." />';
            echo '<p class="description">' . esc_html__('Jeton Utilisateur avec permission publish_to_groups, et l’app doit être ajoutée au Groupe.', 'fbpublishplugin') . '</p>';
        }, self::OPTION_KEY, 'fbpublish_main');
        ...
        add_settings_field('access_token', __('Jeton global (optionnel)', 'fbpublishplugin'), function () {
            ...
            echo '<p class="description">' . esc_html__('Facultatif: utilisé en repli si un jeton spécifique (Page/Groupe) est vide.', 'fbpublishplugin') . '</p>';
        }, self::OPTION_KEY, 'fbpublish_main');
