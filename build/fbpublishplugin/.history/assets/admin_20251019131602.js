      ...
      $.post(FBPUBLISH.ajaxUrl, {
        ...
        if (res && res.success) {
          ...
          if (res.data && res.data.details) {
            ...
            if (d.group) {
              parts.push('groupe: ' + (d.group.id ? ('ok id=' + d.group.id) : (d.group.skipped ? 'déjà publié' : 'ok')));
            }
            if (parts.length) msg += '\n' + parts.join('\n');
          }
          alert(msg);
          ...
          if (res && res.data) {
            ...
            if (res.data.details) {
              ...
              if (d2.group) {
                lines.push('groupe: ' + (d2.group.error ? d2.group.error : 'inconnu') + (d2.group.status ? (' (HTTP ' + d2.group.status + ')') : ''));
              }
              if (lines.length) err += '\n' + lines.join('\n');
            }
          }
