...
class FB_Publish_Plugin {
    ...
    public function __construct() {
        ...
        add_action(self::CRON_HOOK, [$this, 'handle_deferred_share'], 10, 3);
    }
    ...
    public function render_meta_box($post) {
        ...
        $suggest_photo = !empty($thumb_url);
        $checked_photo = ($share_as_photo === '1');
        echo '<p><label><input type="checkbox" name="fbpublish_share_as_photo" value="1" ' . checked(true, $checked_photo, false) . ' ' . disabled(!$suggest_photo, true, false) . ' /> ' . esc_html__('Partager en photo (grand visuel)', 'fbpublishplugin') . '</label></p>';
        if (!$suggest_photo) {
            echo '<p class="description">' . esc_html__('Ajoutez une image mise en avant pour activer le mode photo.', 'fbpublishplugin') . '</p>';
        }
    ...
    public function handle_manual_share_ajax() {
        ...
        $result = $this->share_to_facebook($post_id, $destinations, $message, $force);
        if ($result['success']) {
    ...
    public function maybe_auto_share_on_publish($new_status, $old_status, $post) {
        ...
        if (!wp_next_scheduled(self::CRON_HOOK, [$post_id, $destinations, $custom_message])) {
            wp_schedule_single_event(time() + $delay, self::CRON_HOOK, [$post_id, $destinations, $custom_message]);
    ...
    public function handle_deferred_share($post_id, $destinations, $message_override) {
        $status = get_post_status($post_id);
        if ($status !== 'publish') {
            return;
        }
        $this->share_to_facebook($post_id, (array) $destinations, (string) $message_override, false);
    }
    ...
    private function share_to_destination($post_id, $dest, $message) {
        ...
        // Ask Facebook to (re)scrape the URL so OG image/metadata are fresh
    ...
    public function share_to_facebook($post_id, $destinations, $message_override = '', $force = false) {
        $details = [];
