...
class FB_Publish_Plugin {
    ...
    public function register_meta_box() {
        // Register meta box for all public post types (including CPTs), except "terme"
        $post_types = get_post_types(['public' => true], 'names');
        if (empty($post_types)) {
            $post_types = ['post']; // Fallback to post if no public types found
        }
        // Exclude "terme" post type
        $post_types = array_diff($post_types, ['terme']);
        add_meta_box(
            'fbpublish_meta',
            __('Publication Facebook', 'fbpublishplugin'),
    ...
    public function maybe_auto_share_on_publish($new_status, $old_status, $post) {
        ...
        // Exclude "terme" post type from auto-publishing
        if ($post->post_type === 'terme') {
            return;
        }
        $post_id = $post->ID;
